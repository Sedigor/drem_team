...
def main(path_client=0):
    if path_client:
        ...
        if p.exists():
            ...
            perenos(path)
            del_empty_fold(path)
            messagebox.showinfo("Інформація","Файли відсортовано!")
        else:
            messagebox.showerror("Помилка", "Такої папки не існує!")
    else:
        messagebox.showwarning("Попередження", "Уведіть шлях до папки")
